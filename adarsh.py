...
def add_user(message):
    ...
    if user_id in admin_id:
        ...
        response = "ʏᴏᴜ ᴀʀᴇ ɴᴏᴛ ᴀᴘᴘʀᴏᴠᴇ ʙʏ ᴀᴅᴍɪɴ ᴘʟᴇᴀꜱᴇ ᴄᴏɴᴛᴀᴄᴛ --> @jodsmoker & @JODSMOKER1"
...
def clear_users_command(message):
    ...
    if user_id in admin_id:
        ...
        response = "ʏᴏᴜ ᴀʀᴇ ɴᴏᴛ ᴀᴘᴘʀᴏᴠᴇ ʙʏ ᴀᴅᴍɪɴ ᴘʟᴇᴀꜱᴇ ᴄᴏɴᴛᴀᴄᴛ --> @jodsmoker & that's brother @JODSMOKER1"
    bot.reply_to(message, response)
...
def start_attack_reply(message, target, port, time):
    ...
    response = f"🥰𝐇𝐄𝐘 -> {username} \n🤣🚀 Attack Initiated! 🚀 🤣1--> [𝐒𝐓𝐀𝐑𝐓𝐄𝐃]\n\n†αrgε† -> {target}\np⊕r† -> {port}\n†ïmε -> {time} 𝐒𝐞𝐜𝐨𝐧𝐝𝐬\ngαmε --> 🇮🇳🅑🅖🅜🅘🇮🇳\n\n彡[ 🥰 @jodsmoker & that's brother @JODSMOKER1]彡"
    bot.reply_to(message, response)
...
Example: /attack 34.123.45.67 20000 60

Restrictions:
• 🌐 IP: Must start with 34.
• 🔌 Port: 10000-30000
• ⏱️ Time: 60-1000s"  # Updated command syntax
    else:
        response = ("❌ Access Denied! You are not authorized to use this premium service.*")
...
𝘽𝙪𝙮 :- @jodsmoker
𝙊𝙛𝙛𝙞𝙘𝙞𝙖𝙡 :- @@jodsmoker and that's brother @@JODSMOKER1 
'''
...
def welcome_start(message):
    ...
    response = f'''🌍 WELCOME TO DDOS WORLD! 🎉
...
𝘽𝙪𝙮 :- @MR_ADARSH_YTs
𝙊𝙛𝙛𝙞𝙘𝙞𝙖𝙡 :- @MR_ADARSH_YTs and that's brother @iam666bro 
'''
    for handler in bot.message_handlers:
        if hasattr(handler, 'commands'):
            if message.text.startswith('/help'):
